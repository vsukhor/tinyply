...
This version of the library uses elements of C++23 and so requires a relatively recent compiler (GCC 15.1 is sufficient). _tinyply_ supports exporting and importing PLY files in both binary and ascii formats. It may be used for filesizes >= 4gb and can read big-endian binary files (but not write them).
